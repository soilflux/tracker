function Update() {
	...
	if(Game.theme == "dark" && Game.themeChange) {
		if (Person.isBoring != true) {
			...
			document.getElementById("iWantToRemoveThisButton").innerHTML="CoolCat";
		}
...
if(tempstring.length == 6) 	{
	...
	if(Logic.emerald.length > 0 && Logic.ruby.length > 0 && Logic.sapphire.length> 0 && Logic.emerald != "???") {
...
if(tempstring.length == 6) 	{
	...
	if(Logic.forest_medallion_location.length > 0 && Logic.fire_medallion_location.length > 0 && Logic.water_medallion_location.length> 0 && Logic.forest_medallion_location != "???") {
		parent.removeChild(child);
