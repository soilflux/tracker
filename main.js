	...
	for (var i = 1; i <= 34; i++) {
		...
		if (woth1 == AreaNames[i] || woth2 == AreaNames[i] || woth3 == AreaNames[i] || woth4 == AreaNames[i] || woth5 == AreaNames[i]) {
			...
			for (k = 0; k < tempArray.length; k++) {
				if (ChecksPutInLogicBy[tempArray[k]].length < 15 ||(ChecksPutInLogicBy[tempArray[k]].length < 5 && tempArray[k].startsWith("bomb_bag"))) {tempArray2.push.apply(tempArray2, ChecksPutInLogicBy[tempArray[k]]);}
			}
