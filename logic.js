...
function logicShortcuts() {
	...
	Logic.can_enter_jabu_entrance = ((Logic.rutos_letter || document.getElementById("closedFountain").value == "OPEN") && Logic.child_can_enter_domain) || Spawn.child_zf;
	Logic.can_enter_jabu = Logic["ER_jabu_enter"];
	...
	Logic.gtg_access = Logic["ER_gtg_enter"];
	...
	Logic.dodongos_access = Logic["ER_dc_enter"];
	Logic.can_break_dodongos_wall = Logic.dodongos_access && (Logic.can_blast_or_smash || Logic.goron_bracelet);
	...
	Logic.shadow_temple_access = Logic["ER_shadow_enter"];
	Logic.can_cross_shadow_gap= Logic.shadow_temple_access && Logic.hover_boots;
	...
	Logic.ice_access = Logic["ER_ice_enter"];
	...
	Logic.fire_temple_access = Logic["ER_fire_enter"];
	...
	Logic.adult_spirit_access = Logic.spirit_access && Logic.silver_gauntlets;
	...
	Logic.can_climb_fire_temple = Logic.fire_temple_access && Logic.fire_keys >=3 && Logic.can_wear_goron_tunic && Logic.goron_bracelet && (Logic.bow || Logic.hookshot || Logic.bomb_bag);
	Logic.can_enter_water_entrance = (Logic.hookshot && Logic.iron_boots) || (Logic.longshot && Logic.golden_scale);
	Logic.water_temple_access = Logic["ER_water_enter"];
	Logic.can_do_water_checks = Logic.water_temple_access && Logic.iron_boots;
	...
	Logic.deku_access = Logic["ER_deku_enter"];
	...
	Player.can_enter_jabu_entrance = (Player.child_can_enter_domain && (Player.rutos_letter || document.getElementById("closedFountain").value == "OPEN")) || Spawn.child_zf;
	Player.can_enter_jabu = Player["ER_jabu_enter"];
	...
	Player.gtg_access = Player["ER_gtg_enter"];
	...
	Player.dodongos_access = Player["ER_dc_enter"];
	Player.can_break_dodongos_wall = Player.dodongos_access && (Player.can_blast_or_smash || Player.goron_bracelet || ((Player.can_use_bottle || Player.can_enter_jabu_entrance) && Player.giants_wallet) || Player.can_shoot_blue_fire_arrows);
	...
	Player.shadow_temple_access = Player["ER_shadow_enter"];
	Player.can_cross_shadow_gap= Player.shadow_temple_access && Player.hover_boots;
	...
	Player.ice_access = Player["ER_ice_enter"];
	...
	Player.fire_temple_access = Player["ER_fire_enter"];
	Player.adult_spirit_access = Player.spirit_access && Player.silver_gauntlets;
	...
	Player.can_climb_fire_temple = Player.fire_temple_access && Player.current_fire_keys >=3 && (Player.bow || Player.hookshot || Player.bomb_bag || Player.has_chus);
	Player.can_enter_water_entrance = Player.hookshot && (Player.iron_boots || Player.golden_scale);
	Player.water_temple_access = Player["ER_water_enter"];
	Player.can_do_water_checks = Player.water_temple_access && (Player.iron_boots || Player.longshot);
	...
	Player.deku_access = Player["ER_deku_enter"];
	...
	CouldHave.can_enter_jabu_entrance = (CouldHave.child_can_enter_domain && (CouldHave.rutos_letter || document.getElementById("closedFountain").value == "OPEN")) || Spawn.child_zf;
	CouldHave.can_enter_jabu = CouldHave["ER_jabu_enter"];
	...
	CouldHave.gtg_access = CouldHave["ER_gtg_enter"];
	...
	CouldHave.dodongos_access = CouldHave["ER_dc_enter"];
	CouldHave.can_break_dodongos_wall = CouldHave.dodongos_access && (CouldHave.can_blast_or_smash || CouldHave.goron_bracelet || ((CouldHave.can_use_bottle || CouldHave.can_enter_jabu_entrance) && CouldHave.giants_wallet) || CouldHave.can_shoot_blue_fire_arrows);
	...
	CouldHave.shadow_temple_access = CouldHave["ER_shadow_enter"];
	CouldHave.can_cross_shadow_gap= CouldHave.shadow_temple_access && CouldHave.hover_boots;
	...
	CouldHave.ice_access = CouldHave["ER_ice_enter"];
	...
	CouldHave.fire_temple_access = CouldHave["ER_fire_enter"];
	CouldHave.adult_spirit_access = CouldHave.spirit_access && CouldHave.silver_gauntlets;
	...
	CouldHave.can_climb_fire_temple = CouldHave.fire_temple_access && (Player.current_fire_keys >=3 || (CouldHave.hammer && (CouldHave.bomb_bag || Player.has_chus)))  && (CouldHave.bow || CouldHave.hookshot || CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	CouldHave.can_enter_water_entrance = CouldHave.hookshot && (CouldHave.iron_boots || CouldHave.golden_scale);
	CouldHave.water_temple_access = CouldHave["ER_water_enter"];
	CouldHave.can_do_water_checks = CouldHave.water_temple_access && (CouldHave.iron_boots || CouldHave.longshot);
	...
	CouldHave.deku_access = CouldHave["ER_deku_enter"];
}
function locationLogic(){
	...
	Location_Logic.ice_glacier_hp = Logic.ice_entrance_access;
	Location_Logic.ice_bottom_of_fountain = Logic.ice_entrance_access && Logic.iron_boots;
	...
	Location_Logic.gerudo_archery_1 = Logic.eponas && Logic.bow && Logic.gtg_entrance_access;
	Location_Logic.gerudo_archery_2 = Logic.eponas && Logic.bow && Logic.gtg_entrance_access;
	...
	Location_Logic.forest_first = Logic.forest_temple_access;
	Location_Logic.forest_stalfos = Logic.forest_temple_access;
	Location_Logic.forest_midCourtyard = Logic.forest_temple_access && ((((Logic.time || Logic.bow || (Logic.hover_boots && Logic.forest_keys>=1)) && Logic.hookshot)) || (Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5));
	Location_Logic.forest_highCourtyard = Logic.forest_temple_access && (Logic.time || (Logic.bow && Logic.hookshot) || (Logic.hover_boots && Logic.forest_keys>=1) || (Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5));
	Location_Logic.forest_lowCourtyard = Logic.forest_temple_access && (Logic.time || (Logic.bow && Logic.hookshot) || (Logic.hover_boots && Logic.forest_keys>=1) || (Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5));
	Location_Logic.forest_blockRoom = Logic.forest_temple_access && Logic.forest_keys >= 1 && Logic.bow && Logic.goron_bracelet;
	Location_Logic.forest_bossKey = Logic.forest_temple_access && Logic.forest_keys >= 2 && Logic.bow && Logic.goron_bracelet;
	Location_Logic.forest_floormaster = Logic.forest_temple_access && ((Logic.bow && Logic.goron_bracelet && Logic.forest_keys >=2) || (Logic.hover_boots && Logic.forest_keys >=1));
	Location_Logic.forest_red = Logic.forest_temple_access && Logic.goron_bracelet && Logic.bow && Logic.forest_keys>=3;
	Location_Logic.forest_bow = Logic.forest_temple_access && Logic.goron_bracelet && Logic.forest_keys>=3;
	Location_Logic.forest_blue = Logic.forest_temple_access && Logic.goron_bracelet && Logic.bow && Logic.forest_keys>=3;
	Location_Logic.forest_fallingCeiling = Logic.forest_temple_access && Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5;
	Location_Logic.forest_nearBoss = Logic.forest_temple_access && Logic.goron_bracelet && Logic.bow && Logic.forest_keys>=5;
	Location_Logic.forest_phantomGanon = Logic.forest_temple_access && Logic.goron_bracelet && Logic.bow && Logic.forest_keys>=5 && Logic.forest_boss_key;
	Location_Logic.fire_nearBoss = Logic.fire_temple_access;
	Location_Logic.fire_hammer1 = Logic.fire_temple_access && Logic.hammer && (document.getElementById("keysanity").value != "KEY RINGS" || Logic.fire_key_ring);
	Location_Logic.fire_hammer2 = Logic.fire_temple_access && Logic.hammer && (document.getElementById("keysanity").value != "KEY RINGS" || Logic.fire_key_ring);
	Location_Logic.fire_lavaOpen = Logic.fire_temple_access && Logic.fire_keys >=1;
	Location_Logic.fire_lavaBomb = Logic.fire_temple_access && Logic.fire_keys >=1 && Logic.bomb_bag;
	Location_Logic.fire_volvagia = Logic.fire_temple_access && Logic.fire_boss_key && Logic.hammer && Logic.can_wear_goron_tunic && (Logic.hover_boots || (Logic.fire_keys >=7 && (Logic.time || Logic.bomb_bag)));
	...
	Location_Logic.fire_top = Logic.can_climb_fire_temple && (Logic.fire_keys>=7 || (Logic.fire_keys >= 6 && Logic.hammer && Logic.hover_boots)) && Logic.bomb_bag;
	...
	Location_Logic.water_dLink = Logic.can_do_water_checks && Logic.water_keys >= 5 && Logic.hookshot;
	Location_Logic.water_river = Logic.can_do_water_checks && Logic.water_keys >= 5 && Logic.time && Logic.bow && Logic.hookshot;
	...
	Location_Logic.water_morpha = Logic.can_do_water_checks && Logic.water_boss_key && Logic.longshot ;
	Location_Logic.spirit_childLeft = Logic.child_spirit_access && (Logic.slingshot || Logic.boomerang);
	Location_Logic.spirit_childRight = Logic.child_spirit_access && (Logic.slingshot || Logic.boomerang);
	Location_Logic.spirit_childClimb1 = Logic.spirit_access && (Logic.spirit_keys >=1 && Logic.projectile_both);
	Location_Logic.spirit_childClimb2 = Logic.spirit_access && (Logic.spirit_keys >=1 && Logic.projectile_both);
	Location_Logic.spirit_map = Logic.spirit_access && (Logic.spirit_keys ==5 && Logic.child_spirit_access && Logic.bomb_bag) || (Logic.spirit_keys >=3 && Logic.silver_gauntlets && Logic.can_use_fire) || (Logic.bomb_bag && Logic.spirit_keys >=1 && Logic.can_use_fire);
	Location_Logic.spirit_sunRoom = Logic.spirit_access && (Logic.spirit_keys ==5 && Logic.child_spirit_access && Logic.bomb_bag) || (Logic.spirit_keys >=3 && Logic.silver_gauntlets && Logic.can_use_fire) || (Logic.bomb_bag && Logic.spirit_keys >=1 && Logic.can_use_fire);
	Location_Logic.spirit_rightHand = Logic.spirit_access && ((Logic.spirit_keys >=3 && Logic.longshot && Logic.bomb_bag) || Logic.spirit_keys == 5) && ((Logic.child_spirit_access && Logic.bomb_bag) || (Logic.adult_spirit_access && (Logic.bow || Logic.hookshot || Logic.bomb_bag)));
	Location_Logic.spirit_adultLeft = Logic.adult_spirit_access && Logic.hookshot && Logic.lullaby;
	Location_Logic.spirit_adultRight = Logic.adult_spirit_access && (Logic.bow || Logic.hookshot || Logic.bomb_bag);
	Location_Logic.spirit_rotatingMirror1 = Logic.adult_spirit_access && Logic.spirit_keys >=3;
	Location_Logic.spirit_rotatingMirror2 = Logic.adult_spirit_access && Logic.spirit_keys >=3;
	Location_Logic.spirit_lullabyHand = Logic.adult_spirit_access && Logic.spirit_keys >=3 && Logic.lullaby;
	Location_Logic.spirit_lullabyHigh = Logic.adult_spirit_access && Logic.spirit_keys >=3 && Logic.lullaby && (Logic.hookshot || Logic.hover_boots);
	Location_Logic.spirit_nearFourArmos = Logic.adult_spirit_access && Logic.spirit_keys >=4 && Logic.bomb_bag && Logic.mirror_shield;
	Location_Logic.spirit_invisible1 = Logic.adult_spirit_access && Logic.spirit_keys >=4 && Logic.bomb_bag ;//&& Logic.can_see;
	Location_Logic.spirit_invisible2 = Logic.adult_spirit_access && Logic.spirit_keys >=4 && Logic.bomb_bag ;//&& Logic.can_see;
	Location_Logic.spirit_leftHand = Logic.adult_spirit_access && Logic.spirit_keys >=4 && Logic.bomb_bag;
	Location_Logic.spirit_bossKey = Logic.adult_spirit_access && Logic.spirit_keys ==5 && Logic.bow && Logic.lullaby && Logic.hookshot;
	Location_Logic.spirit_tippyTop = Logic.adult_spirit_access && Logic.spirit_keys ==5 && Logic.mirror_shield;
	Location_Logic.spirit_twinrova = Logic.adult_spirit_access && Logic.spirit_keys ==5 && Logic.mirror_shield && Logic.bomb_bag && Logic.spirit_boss_key && Logic.hookshot;
	Location_Logic.shadow_map = Logic.shadow_temple_access && (Logic.hover_boots || Logic.hookshot);
	Location_Logic.shadow_hovers = Logic.shadow_temple_access && (Logic.hover_boots || Logic.hookshot);
	...
	Location_Logic.shadow_dins1 = Logic.can_ride_shadow_boat && Logic.shadow_keys >=4 && Logic.can_use_dins;
	Location_Logic.shadow_dins2 = Logic.can_ride_shadow_boat && Logic.shadow_keys >=4 && Logic.can_use_dins;
	...
	Location_Logic.gtg_lobbyLeft = Logic.gtg_access && Logic.bow;
	Location_Logic.gtg_lobbyRight = Logic.gtg_access && Logic.bow;
	Location_Logic.gtg_stalfos = Logic.gtg_access;
	Location_Logic.gtg_wolfos = Logic.gtg_access && Logic.hookshot;
	Location_Logic.gtg_silvers1 = Logic.gtg_access && Logic.hookshot && Logic.silver_gauntlets ;//&& Logic.can_see ;
	Location_Logic.gtg_silvers2 = Logic.gtg_access && Logic.hookshot && Logic.silver_gauntlets ;//&& Logic.can_see ;
	Location_Logic.gtg_silvers3 = Logic.gtg_access && Logic.hookshot && Logic.silver_gauntlets ;//&& Logic.can_see ;
	Location_Logic.gtg_silvers4 = Logic.gtg_access && Logic.hookshot && Logic.silver_gauntlets ;//&& Logic.can_see ;
	Location_Logic.gtg_eyes = Logic.gtg_access && Logic.hookshot && Logic.bow;  //((Logic.bomb_bag || Logic.gtg_keys == 9) && Logic.hammer));
	Location_Logic.gtg_aboveEyes = Logic.gtg_access && Logic.hookshot && Logic.bow ;//&& Logic.can_see;
	Location_Logic.gtg_keese = Logic.gtg_access && Logic.hookshot/*&& Logic.can_see*/;
	Location_Logic.gtg_flamesChest = Logic.gtg_access && Logic.hookshot && Logic.hammer;// && (Logic.can_see || (Logic.bomb_bag || Logic.gtg_keys == 9)) ;
	Location_Logic.gtg_freestanding = Logic.gtg_access && (Logic.gtg_keys == 9 || (Logic.bomb_bag && Logic.time) || (Logic.hookshot /*&& Logic.can_see*/ && Logic.time));
	Location_Logic.gtg_right2 = Logic.gtg_access && (Logic.gtg_keys == 9 || (Logic.bomb_bag && Logic.time) || (Logic.hookshot /*&& Logic.can_see*/ && Logic.time));
	Location_Logic.gtg_right3 = Logic.gtg_access && (Logic.gtg_keys == 9 || (Logic.bomb_bag && Logic.time) || (Logic.hookshot /*&& Logic.can_see*/ && Logic.time));
	...
	Location_Logic.gtg_left3 = Logic.gtg_access && Logic.gtg_keys >= 6;
	Location_Logic.gtg_left4 = Logic.gtg_access && Logic.gtg_keys >= 7;
	Location_Logic.gtg_final = Logic.gtg_access && Logic.gtg_keys >= 9;
	Location_Logic.gtg_toilet = Logic.gtg_access && Logic.iron_boots && Logic.time && Logic.hookshot /*&& Logic.can_see*/;
	Location_Logic.well_fakeRight = Logic.well_access;//&& Logic.can_see;
	Location_Logic.well_centerSmall = Logic.well_access;//&& Logic.can_see;
	Location_Logic.well_backBombable = Logic.well_access && Logic.bomb_bag;
	Location_Logic.well_waterLeft = Logic.well_access && Logic.lullaby;
	Location_Logic.well_coffin = Logic.well_access;
	Location_Logic.well_centerBig = Logic.well_access;//&& Logic.can_see;
	Location_Logic.well_fakeLeft = Logic.well_access;//&& Logic.can_see;
	Location_Logic.well_frontBombable = Logic.well_access && Logic.bomb_bag;
	Location_Logic.well_waterFront = Logic.well_access && Logic.lullaby;
	Location_Logic.well_deadHand = Logic.well_access && Logic.lullaby ;//&& Logic.kokiri_sword;
	Location_Logic.well_invisible = Logic.well_access && Logic.lullaby ;//&& Logic.can_see ;
	Location_Logic.well_locked1 = Logic.well_access && Logic.well_keys == 3 ;//&& Logic.can_see ;
	Location_Logic.well_locked2 = Logic.well_access && Logic.well_keys == 3 ;//&& Logic.can_see ;
	Location_Logic.well_basement = Logic.well_access && (Logic.bomb_bag || (((/*Logic.can_see &&*/ Logic.well_keys == 3) || Logic.can_use_dins) && Logic.goron_bracelet));
	...
	Location_Logic.boleroSpot = Logic.fire_temple_access;
	...
	Location_Logic.shop_domain_TL = Logic[Shop_Logic.shop_domain_TL] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.can_enter_jabu_entrance || (Logic.giants_wallet && Logic.bottle))));
	Location_Logic.shop_domain_TR = Logic[Shop_Logic.shop_domain_TR] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.can_enter_jabu_entrance || (Logic.giants_wallet && Logic.bottle))));
	Location_Logic.shop_domain_BR = Logic[Shop_Logic.shop_domain_BR] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.can_enter_jabu_entrance || (Logic.giants_wallet && Logic.bottle))));
	Location_Logic.shop_domain_BL = Logic[Shop_Logic.shop_domain_BL] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.can_enter_jabu_entrance || (Logic.giants_wallet && Logic.bottle))));
	...
	Location_Logic.gs_fountain_above_log = Logic.can_enter_jabu_entrance && Logic.boomerang;
	Location_Logic.gs_fountain_tree = Logic.can_enter_jabu_entrance;
	Location_Logic.gs_fountain_hidden_cave = Logic.ice_entrance_access && Logic.silver_gauntlets && Logic.hookshot && Logic.can_blast_or_smash;
	...
	Location_Logic.gs_forest_first = Logic.forest_temple_access && Logic.hookshot;
	Location_Logic.gs_forest_lobby = Logic.forest_temple_access && Logic.hookshot;
	Location_Logic.gs_forest_outdoor_east = Logic.forest_temple_access && Logic.hookshot && ((Logic.bow || Logic.time) || (Logic.forest_keys >= 1 && Logic.hover_boots));
	Location_Logic.gs_forest_outdoor_west = Logic.forest_temple_access && Logic.hookshot && (((Logic.bow || Logic.time) && Logic.longshot) || (Logic.forest_keys >= 1 && Logic.hover_boots) || (Logic.forest_keys >= 2 && Logic.goron_bracelet && Logic.bow))
	Location_Logic.gs_forest_basement = Logic.forest_temple_access && Logic.hookshot && Logic.bow && Logic.goron_bracelet && Logic.forest_keys == 5;
	Location_Logic.gs_fire_time = Logic.fire_temple_access && Logic.fire_keys >= 1 && Logic.time;
	...
	Location_Logic.gs_fire_basement = Logic.fire_temple_access && Logic.hammer;
	...
	Location_Logic.gs_spirit_metal_fence = Logic.child_spirit_access && (Logic.boomerang || Logic.slingshot);
	Location_Logic.gs_spirit_before_child_knuckle = Logic.spirit_access && (Logic.bomb_bag && Logic.boomerang && Logic.hookshot && Logic.spirit_keys >= 1) || (Logic.boomerang && Logic.spirit_keys == 5 && Logic.bomb_bag && Logic.child_spirit_access) || (Logic.hookshot && Logic.silver_gauntlets && Logic.spirit_keys >= 3);
	Location_Logic.gs_spirit_boulder_room = Logic.adult_spirit_access && Logic.time && (Logic.bow || Logic.hookshot || Logic.bomb_bag);
	Location_Logic.gs_spirit_lobby = Logic.adult_spirit_access && Logic.spirit_keys >= 3 && (Logic.hookshot || Logic.hover_boots);
	Location_Logic.gs_spirit_child_climb = Logic.spirit_access && Logic.spirit_keys >= 1;
	...
	for(let i = 0, Access = Location_Access, Has = Player; i < 2; i++) {
		...
		Access.poes= (Has.bow && (Has.can_use_bottle || Has.can_enter_jabu_entrance)) || Has.big_poe;
		...
		Access.rolling_goron = Has.bomb_bag || Player.has_chus || Has.goron_bracelet;
		Access.goron_pot = (Has.bomb_bag || Has.goron_bracelet || Player.has_chus) && (Has.lullaby || Has.can_use_dins || Spawn.child_lower_dmc || Spawn.child_lower_dmc_fairy);
		...
		Access.goron_maze_3 = Has.hammer || Has.silver_gauntlets || ((Has.bomb_bag || Player.has_chus) && Has.hover_boots);
		...
		Access.trade_quest = (((Access.thaw_king || (Has.giants_wallet && Has.lullaby && (Has.can_use_bottle || Has.can_enter_jabu_entrance))) && Has.prescription) || Has.claim_check) && Has.crater_top;
		...
		Access.colossus_fairy = Has.can_enter_colossus && (Has.bomb_bag || Player.has_chus) && Has.lullaby;
		...
		Access.dodongos_above_king = Has.dodongos_climb && (Has.bomb_bag || Player.has_chus);
		Access.dodongos_king_dodongo = Has.dodongos_climb && (Has.bomb_bag || Player.has_chus);
		...
		Access.fountain_fairy = ((Has.ice_entrance_access && (Has.bomb_bag || Player.has_chus || (Has.hammer && Has.silver_gauntlets))) || Spawn.child_zf_fairy || Spawn.adult_zf_fairy || ((Has.bomb_bag || Player.has_chus) && Spawn.child_zf)) && Has.lullaby;
		Access.ice_glacier_hp = Has.ice_entrance_access;
		...
		Access.ice_bottom_of_fountain = Has.ice_entrance_access && Has.iron_boots;
		...
		Access.forest_midCourtyard = Has.forest_temple_access && ((((Has.time || Has.bow || ((Has.hover_boots || Has.goron_bracelet) && Player.current_forest_keys>=1)) && Has.hookshot)) || (Has.goron_bracelet && (Has.bow || Has.can_use_dins) && Player.current_forest_keys>=5));
		Access.forest_highCourtyard = Has.forest_temple_access && (Has.time || (Has.bow && Has.hookshot) || ((Has.hover_boots || Has.goron_bracelet) && Player.current_forest_keys>=1) || (Has.goron_bracelet && (Has.bow || Has.can_use_dins) && Player.current_forest_keys>=5));
		Access.forest_lowCourtyard = Has.forest_temple_access && (Has.time || (Has.bow && Has.hookshot) || ((Has.hover_boots || Has.goron_bracelet) && Player.current_forest_keys>=1) || (Has.goron_bracelet && (Has.bow || Has.can_use_dins) && Player.current_forest_keys>=5));
		Access.forest_blockRoom = Has.forest_temple_access && Player.current_forest_keys >= 1 && Has.bow && Has.goron_bracelet && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_bossKey = Has.forest_temple_access && Player.current_forest_keys >= 2 && Has.bow && Has.goron_bracelet && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_floormaster = Has.forest_temple_access && ((Has.bow && Has.goron_bracelet && Player.current_forest_keys >=2) || ((Has.hover_boots || Has.goron_bracelet) && Player.current_forest_keys >=1)) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_red = Has.forest_temple_access && Has.goron_bracelet && Has.bow && Player.current_forest_keys>=3 && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_bow = Has.forest_temple_access && Has.goron_bracelet && Player.current_forest_keys>=3 && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_blue = Has.forest_temple_access && Has.goron_bracelet && Has.bow && Player.current_forest_keys>=3 && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_fallingCeiling = Has.forest_temple_access && Has.goron_bracelet && (Has.bow || Has.can_use_dins) && Player.current_forest_keys>=5 && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_nearBoss = Has.forest_temple_access && Has.goron_bracelet && Has.bow && Player.current_forest_keys>=5 && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.forest_phantomGanon = Has.forest_temple_access && Has.goron_bracelet && Has.bow && Player.current_forest_keys>=5 && Player.forest_boss_key && (document.getElementById("keysanity").value != "KEY RINGS" || Has.forest_key_ring);
		Access.fire_nearBoss = Has.fire_temple_access;
		Access.fire_hammer1 = Has.fire_temple_access && Has.hammer && (document.getElementById("keysanity").value != "KEY RINGS" || Has.fire_key_ring);
		Access.fire_hammer2 = Has.fire_temple_access && Has.hammer && (document.getElementById("keysanity").value != "KEY RINGS" || Has.fire_key_ring);
		...
		Access.fire_lavaBomb = Has.fire_temple_access && Player.current_fire_keys >=1 && (Has.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.fire_key_ring);
		Access.fire_volvagia = Has.fire_temple_access && Player.fire_boss_key && Has.hammer && (document.getElementById("keysanity").value != "KEY RINGS" || Has.fire_key_ring);
		...
		Access.fire_shortcut = Has.can_climb_fire_temple && Player.current_fire_keys>=5 && (Has.bomb_bag || Player.has_chus);
		...
		Access.fire_sotGoron = Has.can_climb_fire_temple && (Player.current_fire_keys>=7 || (Player.current_fire_keys>=6)) && Has.hammer && (Has.time || Has.bomb_bag || Player.has_chus);
		Access.fire_top = Has.can_climb_fire_temple && (Player.current_fire_keys>=7 || (Player.current_fire_keys >= 6)) && (Has.bomb_bag || Player.has_chus);
		...
		Access.water_cracked = Has.can_do_water_checks && (Has.lullaby || (Has.iron_boots && Player.has_chus)) && (Has.bomb_bag || Player.has_chus);
		...
		Access.spirit_childClimb2 = Has.spirit_access && Player.current_spirit_keys >= 1 && ((Has.projectile_child && Has.child_spirit_access) || (Has.projectile_adult && Has.silver_gauntlets)) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_map = Has.spirit_access && Player.current_spirit_keys >= 1 && ((Has.child_spirit_access && (Has.bomb_bag || Player.has_chus)) || (((Has.can_use_fire || Has.bow) && Has.silver_gauntlets))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_sunRoom = Has.spirit_access && Player.current_spirit_keys >= 1 && ((Has.child_spirit_access && (Has.bomb_bag || Player.has_chus)) || (((Has.can_use_fire || Has.bow) && Has.silver_gauntlets))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_rightHand = Has.spirit_access && Player.current_spirit_keys >= 2 && ((Has.child_spirit_access && (Has.bomb_bag || Player.has_chus)) || (Has.adult_spirit_access && (Has.bow || Has.hookshot || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_adultLeft = Has.adult_spirit_access && Has.hookshot && Has.lullaby;
		Access.spirit_adultRight = Has.adult_spirit_access && (Has.bow || Has.hookshot || Player.has_chus);
		Access.spirit_rotatingMirror1 = Has.adult_spirit_access && Player.current_spirit_keys >=1 && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_rotatingMirror2 = Has.adult_spirit_access && Player.current_spirit_keys >=1 && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_lullabyHand = Has.adult_spirit_access && Player.current_spirit_keys >=1 && Has.lullaby && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_lullabyHigh = Has.adult_spirit_access && Player.current_spirit_keys >=1 && Has.lullaby && (Has.hookshot || Has.hover_boots) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_nearFourArmos = Has.adult_spirit_access && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && Has.mirror_shield && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_invisible1 = Has.adult_spirit_access && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring) ;//&& Has.can_see;
		Access.spirit_invisible2 = Has.adult_spirit_access && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring) ;//&& Has.can_see;
		Access.spirit_leftHand = Has.adult_spirit_access && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_bossKey = Has.adult_spirit_access && Player.current_spirit_keys >=3 && Has.lullaby && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_tippyTop = Has.adult_spirit_access && Player.current_spirit_keys >=3 && Has.mirror_shield && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_twinrova = Has.adult_spirit_access && Player.current_spirit_keys >=3 && Has.mirror_shield && (Has.bomb_bag || Player.has_chus) && Player.spirit_boss_key && Has.hookshot && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.shadow_map = Has.shadow_temple_access && (Has.hover_boots || Has.hookshot);
		Access.shadow_hovers = Has.shadow_temple_access && (Has.hover_boots || Has.hookshot);
		...
		Access.ganons_spiritTrial2 = Has.can_enter_ganons && (Player.has_chus || Has.bow);//&& Has.can_see;
		...
		Access.gtg_lobbyLeft = Has.gtg_access && Has.bow;
		Access.gtg_lobbyRight = Has.gtg_access && Has.bow;
		Access.gtg_stalfos = Has.gtg_access;
		Access.gtg_wolfos = Has.gtg_access;
		Access.gtg_silvers1 = Has.gtg_access && Has.can_climb_gtg_hole && Has.silver_gauntlets ;//&& Has.can_see ;
		Access.gtg_silvers2 = Has.gtg_access && Has.can_climb_gtg_hole && Has.silver_gauntlets ;//&& Has.can_see ;
		Access.gtg_silvers3 = Has.gtg_access && Has.can_climb_gtg_hole && Has.silver_gauntlets ;//&& Has.can_see ;
		Access.gtg_silvers4 = Has.gtg_access && Has.can_climb_gtg_hole && Has.silver_gauntlets ;//&& Has.can_see ;
		Access.gtg_eyes = Has.gtg_access && Has.can_climb_gtg_hole && Has.bow;  //((Has.bomb_bag || Player.current_gtg_keys == 9) && Has.hammer));
		Access.gtg_aboveEyes = Has.gtg_access && Has.can_climb_gtg_hole && Has.bow;//&& Has.can_see;
		Access.gtg_keese = Has.gtg_access && Has.can_climb_gtg_hole;// && (Has.can_see || (Has.bomb_bag || Player.current_gtg_keys == 9)) ;
		Access.gtg_flamesChest = Has.gtg_access && Has.can_climb_gtg_hole/*&& Has.can_see*/;
		Access.gtg_freestanding = Has.gtg_access && (Player.current_gtg_keys >= 2 || ((Has.bomb_bag || Player.has_chus) && Has.time) || (Has.can_climb_gtg_hole /*&& Has.can_see*/ && Has.time));
		Access.gtg_right2 = Has.gtg_access && (Player.current_gtg_keys >= 2 || ((Has.bomb_bag || Player.has_chus) && Has.time) || (Has.can_climb_gtg_hole /*&& Has.can_see*/ && Has.time));
		Access.gtg_right3 = Has.gtg_access && (Player.current_gtg_keys >= 2 || ((Has.bomb_bag || Player.has_chus) && Has.time) || (Has.can_climb_gtg_hole /*&& Has.can_see*/ && Has.time));
		Access.gtg_beamos = Has.gtg_access && (Has.bomb_bag || Player.has_chus);
		...
		Access.gtg_toilet = Has.gtg_access && Has.iron_boots && Has.time && Has.hookshot /*&& Has.can_see*/;
		Access.well_fakeRight = Has.well_access;//&& Has.can_see;
		Access.well_backBombable = Has.well_access && (Has.bomb_bag || Player.has_chus);
		Access.well_waterLeft = Has.well_access && Has.lullaby;
		Access.well_coffin = Has.well_access;
		Access.well_centerSmall = Has.well_access;//&& Has.can_see;
		Access.well_centerBig = Has.well_access;//&& Has.can_see;
		Access.well_frontBombable = Has.well_access && (Has.bomb_bag || Player.has_chus);
		Access.well_deadHand = Has.well_access && Has.lullaby ;//&& Has.kokiri_sword;
		Access.well_invisible = Has.well_access && Has.lullaby ;//&& Has.can_see ;
		Access.well_waterFront = Has.well_access && Has.lullaby;
		Access.well_fakeLeft = Has.well_access;//&& Has.can_see;
		Access.well_locked1 = Has.well_access && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);//&& Has.can_see ;
		Access.well_locked2 = Has.well_access &&  (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);//&& Has.can_see ;
		Access.well_basement = Has.well_access && ((Has.bomb_bag || Player.has_chus) || (((/*Has.can_see &&*/ Player.current_well_keys >= 1) || Has.can_use_dins) && Has.goron_bracelet));
		Access.zeldasSpot = true;
		Access.eponasSpot = true;
		Access.sariasSpot = true;
		Access.stormsSpot = true;
		Access.sunsSpot = Has.lullaby;
		Access.boleroSpot = Has.fire_temple_access;
		...
		Access.scrub_dodongos_3 = Has.dodongos_climb && (Has.bomb_bag || Player.has_chus || Has.goron_bracelet);
		Access.scrub_dodongos_4 = Has.dodongos_climb && (Has.bomb_bag || Player.has_chus || Has.goron_bracelet);
		...
		Access.scrub_crater_child = Has.bomb_bag || Spawn.child_upper_dmc || Spawn.child_lower_dmc || Spawn.child_lower_dmc_fairy || Spawn.child_dmt_fairy || Spawn.child_dmt_fairy_ool || Has.hammer || Player.has_chus;
		...
		Access.shop_goron_TL = Has.lullaby || Has.can_stop_link_the_goron || Has.bomb_bag || Has.can_use_dins || Player.has_chus;
		Access.shop_goron_TR = Has.lullaby || Has.can_stop_link_the_goron || Has.bomb_bag || Has.can_use_dins || Player.has_chus;
		Access.shop_goron_BR = Has.lullaby || Has.can_stop_link_the_goron || Has.bomb_bag || Has.can_use_dins || Player.has_chus;
		Access.shop_goron_BL = Has.lullaby || Has.can_stop_link_the_goron || Has.bomb_bag || Has.can_use_dins || Player.has_chus;
		Access.shop_domain_TL = (Has.bomb_bag && Has.lullaby) || Has.silver_scale || (Has.lullaby && ((Has.can_enter_ganons && Has.can_use_bottle) || Has.can_enter_jabu_entrance || (Has.giants_wallet && Has.can_use_bottle)));
		Access.shop_domain_TR = (Has.bomb_bag && Has.lullaby) || Has.silver_scale || (Has.lullaby && ((Has.can_enter_ganons && Has.can_use_bottle) || Has.can_enter_jabu_entrance || (Has.giants_wallet && Has.can_use_bottle)));
		Access.shop_domain_BR = (Has.bomb_bag && Has.lullaby) || Has.silver_scale || (Has.lullaby && ((Has.can_enter_ganons && Has.can_use_bottle) || Has.can_enter_jabu_entrance || (Has.giants_wallet && Has.can_use_bottle)));
		Access.shop_domain_BL = (Has.bomb_bag && Has.lullaby) || Has.silver_scale || (Has.lullaby && ((Has.can_enter_ganons && Has.can_use_bottle) || Has.can_enter_jabu_entrance || (Has.giants_wallet && Has.can_use_bottle)));
		...
		Access.gs_lost_woods_above_stage = Has.bean_access || (Has.hookshot && (Player.has_chus || Has.bow)) || Has.longshot;
		Access.gs_sacred_forest = Has.hookshot;
		Access.gs_outside_kakariko = (Has.boomerang && (Has.bomb_bag || Player.has_chus)) || (Has.can_blast_or_smash && Has.hookshot);
		Access.gs_near_gerudo = (Has.hammer && Has.can_use_fire && Has.hookshot) || (Has.can_use_dins && (Has.bomb_bag || Player.has_chus) && Has.boomerang);
		...
		Access.gs_trail_soil = Has.can_use_bottle && ((Has.bomb_bag || Player.has_chus) || Has.goron_bracelet);
		...
		Access.gs_goron_maze = Has.bomb_bag || Player.has_chus;
		...
		Access.gs_fountain_above_log = Has.can_enter_jabu_entrance && Has.boomerang;
		Access.gs_fountain_tree = Has.can_enter_jabu_entrance;
		...
		Access.gs_ogc = true;
		Access.gs_deku_basement_back = Has.deku_access && Has.boomerang && (Has.bomb_bag || Player.has_chus);
		...
		Access.gs_dodongos_before_king = (Has.bomb_bag || Player.has_chus);
		...
		Access.gs_forest_first = Has.forest_temple_access && Has.hookshot;
		Access.gs_forest_lobby = Has.forest_temple_access && Has.hookshot;
		Access.gs_forest_outdoor_east = Has.forest_temple_access && Has.hookshot && ((Has.bow || Has.time) || (Player.current_forest_keys >= 1 && Has.hover_boots));
		Access.gs_forest_outdoor_west = Has.forest_temple_access && Has.hookshot && (((Has.bow || Has.time) && Has.longshot) || (Player.current_forest_keys >= 1 && Has.hover_boots) || (Player.current_forest_keys >= 2 && Has.goron_bracelet && Has.bow))
		Access.gs_forest_basement = Has.forest_temple_access && Has.hookshot && Has.bow && Has.goron_bracelet && Player.current_forest_keys == 5;
		Access.gs_fire_time = Has.fire_temple_access && Player.current_fire_keys >= 1;
		Access.gs_fire_bomb_wall = Has.can_climb_fire_temple && (Has.bomb_bag || Player.has_chus);
		...
		Access.gs_fire_basement = Has.fire_temple_access && Has.hammer;
		Access.gs_water_south_basement = Has.can_do_water_checks && (Has.bomb_bag || Player.has_chus) && Has.lullaby && Has.hookshot;
		...
		Access.gs_spirit_metal_fence = Has.child_spirit_access && (Has.boomerang || Has.slingshot || Player.has_chus);
		Access.gs_spirit_before_child_knuckle = Has.spirit_access && ((Has.bomb_bag || Player.has_chus) && Has.boomerang && Player.current_spirit_keys >= 1 && Has.child_spirit_access) || (Has.hookshot && Has.silver_gauntlets && Player.current_spirit_keys >= 1);
		Access.gs_spirit_boulder_room = Has.adult_spirit_access && Has.time && (Has.bow || Has.hookshot || Player.has_chus);
		Access.gs_spirit_lobby = Has.adult_spirit_access && Player.current_spirit_keys >= 1 && (Has.hookshot || Has.hover_boots);
		Access.gs_spirit_child_climb = Has.spirit_access && Player.current_spirit_keys >= 1;
		...
		Access.gs_well_west_inner = Has.well_access && Has.boomerang && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);
		Access.gs_well_east_inner = Has.well_access && Has.boomerang && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);
		Access.gs_well_like_like = Has.well_access && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);
	...
	Location_Could_Access.forest_first = CouldHave.forest_temple_access;
	Location_Could_Access.forest_stalfos = CouldHave.forest_temple_access;
	Location_Could_Access.forest_midCourtyard = CouldHave.forest_temple_access && CouldHave.hookshot && (CouldHave.time || CouldHave.bow || ((CouldHave.hover_boots || CouldHave.goron_bracelet) && Player.current_forest_keys>=1));
	Location_Could_Access.forest_highCourtyard = CouldHave.forest_temple_access && (CouldHave.time || ((CouldHave.hover_boots || CouldHave.goron_bracelet) && Player.current_forest_keys>=1) || (CouldHave.bow/* && (CouldHave.iron_boots || CouldHave.scale2 || CouldHave.longshot)*/));
	Location_Could_Access.forest_lowCourtyard = CouldHave.forest_temple_access && (CouldHave.time || ((CouldHave.hover_boots || CouldHave.goron_bracelet) && Player.current_forest_keys>=1) || (CouldHave.bow/* && (CouldHave.iron_boots || CouldHave.scale2 || CouldHave.longshot)*/));
	Location_Could_Access.forest_blockRoom = CouldHave.forest_temple_access && CouldHave.bow && CouldHave.goron_bracelet && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_bossKey = CouldHave.forest_temple_access && CouldHave.bow && CouldHave.goron_bracelet && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_floormaster = CouldHave.forest_temple_access && ((CouldHave.bow && CouldHave.goron_bracelet) || ((CouldHave.hover_boots || CouldHave.goron_bracelet) && Player.current_forest_keys >=1)) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_red = CouldHave.forest_temple_access && CouldHave.goron_bracelet && CouldHave.bow && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_bow = CouldHave.forest_temple_access && CouldHave.goron_bracelet && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_blue = CouldHave.forest_temple_access && CouldHave.goron_bracelet && CouldHave.bow && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_fallingCeiling = CouldHave.forest_temple_access && CouldHave.goron_bracelet && (CouldHave.bow || CouldHave.can_use_dins) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_nearBoss = CouldHave.forest_temple_access && CouldHave.goron_bracelet && CouldHave.bow && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.forest_phantomGanon = CouldHave.forest_temple_access && CouldHave.goron_bracelet && CouldHave.bow && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.forest_key_ring);
	Location_Could_Access.fire_nearBoss = CouldHave.fire_temple_access;
	Location_Could_Access.fire_hammer1 = CouldHave.fire_temple_access && CouldHave.hammer && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	Location_Could_Access.fire_hammer2 = CouldHave.fire_temple_access && CouldHave.hammer && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	Location_Could_Access.fire_lavaOpen = CouldHave.fire_temple_access && (Player.current_fire_keys >=1 || CouldHave.hammer) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	Location_Could_Access.fire_lavaBomb = CouldHave.fire_temple_access && (Player.current_fire_keys >=1 || CouldHave.hammer) && (CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	Location_Could_Access.fire_volvagia = CouldHave.fire_temple_access && ((Player.fire_boss_key && CouldHave.hammer) || (CouldHave.hammer && CouldHave.bow && CouldHave.hookshot && (CouldHave.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	...
	Location_Could_Access.fire_upperMaze = CouldHave.can_climb_fire_temple && (Player.current_fire_keys>=5 || (CouldHave.bow && (CouldHave.bomb_bag || Player.has_chus)));
	Location_Could_Access.fire_shortcut = CouldHave.can_climb_fire_temple && ((Player.current_fire_keys>=5 && (CouldHave.bomb_bag || Player.has_chus)) || (CouldHave.hammer && CouldHave.bow && (CouldHave.bomb_bag || Player.has_chus)));
	Location_Could_Access.fire_scarecrow = CouldHave.can_climb_fire_temple && ((Player.current_fire_keys>=5 && CouldHave.hookshot) || (CouldHave.hammer && CouldHave.bow && CouldHave.hookshot && (CouldHave.bomb_bag || Player.has_chus)));
	Location_Could_Access.fire_compass = CouldHave.can_climb_fire_temple && (Player.current_fire_keys>=6 || (CouldHave.hammer && CouldHave.bow && CouldHave.hookshot && (CouldHave.bomb_bag || Player.has_chus)));
	Location_Could_Access.fire_sotGoron = CouldHave.can_climb_fire_temple && ((Player.current_fire_keys>=7 || (Player.current_fire_keys>=6)) && CouldHave.hammer && (CouldHave.time || CouldHave.bomb_bag || Player.has_chus || CouldHave.hover_boots) || (CouldHave.hammer && CouldHave.bow && CouldHave.hookshot && (CouldHave.bomb_bag || Player.has_chus)));
	Location_Could_Access.fire_top = CouldHave.can_climb_fire_temple && ((Player.current_fire_keys>=7 || (Player.current_fire_keys >= 6)) && (CouldHave.bomb_bag || Player.has_chus) || (CouldHave.hammer && CouldHave.bow && CouldHave.hookshot && (CouldHave.bomb_bag || Player.has_chus)));
	Location_Could_Access.water_compass = CouldHave.can_do_water_checks && (CouldHave.lullaby || CouldHave.iron_boots);
	Location_Could_Access.water_map = CouldHave.can_do_water_checks;
	Location_Could_Access.water_cracked = CouldHave.can_do_water_checks && (CouldHave.lullaby || (CouldHave.iron_boots && Player.has_chus)) && (CouldHave.bomb_bag || Player.has_chus);
	...
	Location_Could_Access.water_dLink = CouldHave.can_do_water_checks && CouldHave.hookshot && (Player.current_water_keys >= 2 || (CouldHave.lullaby && CouldHave.bow && CouldHave.goron_bracelet && (CouldHave.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.water_key_ring);
	...
	Location_Could_Access.spirit_map = (Player.current_spirit_keys >= 1 && ((CouldHave.child_spirit_access && (CouldHave.bomb_bag || Player.has_chus)) || (((CouldHave.can_use_fire || CouldHave.bow) && CouldHave.silver_gauntlets)))) || (Location_Could_Access.spirit_childLeft && Location_Could_Access.spirit_childRight && Location_Could_Access.spirit_adultLeft && Location_Could_Access.spirit_adultRight && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring));
	Location_Could_Access.spirit_sunRoom = (Player.current_spirit_keys >= 1 && ((CouldHave.child_spirit_access && (CouldHave.bomb_bag || Player.has_chus)) || (((CouldHave.can_use_fire || CouldHave.bow) && CouldHave.silver_gauntlets)))) || (Location_Could_Access.spirit_childLeft && Location_Could_Access.spirit_childRight && Location_Could_Access.spirit_adultLeft && Location_Could_Access.spirit_adultRight) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_rightHand = Player.current_spirit_keys >= 2 && ((CouldHave.adult_spirit_access && CouldHave.projectile_adult) || (CouldHave.child_spirit_access && (CouldHave.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_adultLeft = CouldHave.adult_spirit_access && CouldHave.hookshot && CouldHave.lullaby;
	Location_Could_Access.spirit_adultRight = CouldHave.adult_spirit_access && (CouldHave.bow || CouldHave.hookshot || (CouldHave.bomb_bag || Player.has_chus));
	Location_Could_Access.spirit_rotatingMirror1 = CouldHave.adult_spirit_access && (Player.current_spirit_keys >=1) || (Location_Could_Access.spirit_childLeft && Location_Could_Access.spirit_childRight && Location_Could_Access.spirit_adultLeft && Location_Could_Access.spirit_adultRight) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_rotatingMirror2 = CouldHave.adult_spirit_access && (Player.current_spirit_keys >=1) || (Location_Could_Access.spirit_childLeft && Location_Could_Access.spirit_childRight && Location_Could_Access.spirit_adultLeft && Location_Could_Access.spirit_adultRight) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_lullabyHand = CouldHave.adult_spirit_access && (Player.current_spirit_keys >=1 || (Location_Could_Access.spirit_childLeft && Location_Could_Access.spirit_childRight && Location_Could_Access.spirit_adultLeft && Location_Could_Access.spirit_adultRight)) && CouldHave.lullaby && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_lullabyHigh = CouldHave.adult_spirit_access && (Player.current_spirit_keys >=1 || (Location_Could_Access.spirit_childLeft && Location_Could_Access.spirit_childRight && Location_Could_Access.spirit_adultLeft && Location_Could_Access.spirit_adultRight)) && CouldHave.lullaby && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_nearFourArmos = CouldHave.adult_spirit_access && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && CouldHave.mirror_shield && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_invisible1 = CouldHave.adult_spirit_access && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring); //&& CouldHave.can_see;
	Location_Could_Access.spirit_invisible2 = CouldHave.adult_spirit_access && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring); //&& CouldHave.can_see;
	Location_Could_Access.spirit_leftHand = CouldHave.adult_spirit_access && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_bossKey = CouldHave.adult_spirit_access && Player.current_spirit_keys >=3 && CouldHave.lullaby && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_tippyTop = CouldHave.adult_spirit_access && Player.current_spirit_keys >=3 && CouldHave.mirror_shield && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_twinrova = CouldHave.adult_spirit_access && Player.current_spirit_keys >=3 && CouldHave.mirror_shield && (CouldHave.bomb_bag || Player.has_chus) && Player.spirit_boss_key && CouldHave.hookshot && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.shadow_map = CouldHave.shadow_temple_access && (CouldHave.hover_boots || CouldHave.hookshot);
	Location_Could_Access.shadow_hovers = CouldHave.shadow_temple_access && (CouldHave.hover_boots || CouldHave.hookshot);
	...
	Location_Could_Access.shadow_redeadSilvers = CouldHave.can_bomb_shadow_wall;
	Location_Could_Access.shadow_pot = CouldHave.can_bomb_shadow_wall && CouldHave.hookshot;
	Location_Could_Access.shadow_wind = CouldHave.can_pass_shadow_hookshot_door;
	Location_Could_Access.shadow_bombable = CouldHave.can_pass_shadow_hookshot_door;
	Location_Could_Access.shadow_gibdos = CouldHave.can_pass_shadow_hookshot_door;
	Location_Could_Access.shadow_dins1 = CouldHave.can_ride_shadow_boat && CouldHave.can_use_dins;
	...
	for(let i = 0, Peek = Location_Peek, Has = Player; i < 2; i++) {
		...
		Peek.ice_bottom_of_fountain = Has.ice_entrance_access;
		...
		Peek.h_hyrule_web = (Has.hammer && Has.can_use_fire) || ((Has.bomb_bag || Player.has_chus) && Has.can_use_dins)
		Peek.h_valley = true;
		Peek.h_lab = true;
		...
		Peek.h_castle_sos = Has.storms && (Has.bomb_bag || Player.has_chus);
		Peek.h_fountain_fairy = Has.ice_entrance_access || Has.can_enter_jabu_entrance;
		Peek.h_fountain = Has.ice_entrance_access || Has.can_enter_jabu_entrance;
		...
		Peek.h_crater_wall = Has.bomb_bag || Player.has_chus;
		...
		Peek.gs_kokiri_adult = Has.hookshot || Has.hover_boots || Has.bow || Player.has_chus || Has.bomb_bag || Has.can_use_dins;
		Peek.gs_lost_woods_above_stage = Has.bean_access || Player.has_chus || Has.bow || Has.longshot || Has.can_use_dins;
		Peek.gs_sacred_forest = Has.hookshot || Player.has_chus || Has.bomb_bag || Has.bow || Has.can_use_dins;
		Peek.gs_outside_kakariko = ((Has.boomerang || Has.slingshot || Has.can_use_dins) && (Has.bomb_bag || Player.has_chus)) || (Has.can_blast_or_smash && (Has.hookshot || Has.bow || Has.can_use_dins)) || Player.has_chus || Has.bomb_bag;
		Peek.gs_near_gerudo = (Has.hammer && (Has.can_use_fire || Player.has_chus || Has.bow)) || ((Has.bomb_bag || Player.has_chus) && (Has.can_use_dins || Has.slingshot || Player.has_chus));
		Peek.gs_hyrule_castle_grotto = Has.storms && (Has.boomerang || Player.has_chus || Has.slingshot || Has.can_use_dins || Has.bomb_bag);
		Peek.gs_lon_lon_window = Has.boomerang || Player.has_chus || Has.slingshot;
		Peek.gs_lon_lon_back_wall = Has.boomerang || Player.has_chus || Has.slingshot || Has.can_use_dins || Has.bomb_bag;
		Peek.gs_kakariko_impas = Has.hookshot || Spawn.adult_kak_roof || Has.hover_boots || Player.has_chus;
		Peek.gs_graveyard_wall = Has.boomerang || Player.has_chus || Has.slingshot || Has.can_use_dins || Has.bomb_bag;
		Peek.gs_river_near_grotto = Has.hookshot || Player.has_chus || Has.bow || Has.can_use_dins || Has.bomb_bag;
		Peek.gs_river_above_bridge = Has.hookshot || Player.has_chus || Has.bow;
		Peek.gs_fountain_above_log = Has.can_enter_jabu_entrance && (Has.boomerang || Player.has_chus || Has.slingshot || Has.can_use_dins || Has.bomb_bag);
		Peek.gs_fountain_hidden_cave = Has.ice_entrance_access && Has.silver_gauntlets && (Has.hookshot || Player.has_chus || Has.bow || Has.can_use_dins || Has.bomb_bag) && Has.can_blast_or_smash;
		Peek.gs_hylia_tree = Has.longshot || Player.has_chus;
		Peek.gs_valley_small_bridge = true;
		Peek.gs_valley_tent = Has.fortress_access && (Has.hookshot || Player.has_chus || Has.bow || Has.can_use_dins || Has.bomb_bag);
		Peek.gs_valley_pillar = Has.fortress_access && (Has.hookshot || Player.has_chus || Has.bow);
		Peek.gs_fortress_archery = Has.fortress_access && (Has.hookshot || Player.has_chus || Has.bow);
		Peek.gs_wasteland = (Has.hookshot || Player.has_chus || Has.bow || Has.can_use_dins || Has.bomb_bag) && (Has.can_cross_quicksand || Has.can_enter_colossus);
		Peek.gs_colossus_tree = (Has.hookshot || Player.has_chus || Has.bow || Has.can_use_dins || Has.bomb_bag) && Has.can_enter_colossus;
		Peek.gs_colossus_hill = (Has.requiem && Has.bean_access) || ((Has.hookshot || Player.has_chus || Has.bow) && Has.can_enter_colossus);
		Peek.gs_deku_basement_back = Has.deku_access && (Has.bomb_bag || Player.has_chus);
		Peek.gs_dodongos_above_stairs = Has.dodongos_climb && (Has.hookshot || Has.boomerang || Player.has_chus || Has.slingshot || Has.bow || Has.dins_fire || Has.bomb_bag);
		...
		Peek.gs_forest_outdoor_west = Has.forest_temple_access && (Has.hookshot || Has.bow || (Has.time && Player.has_chus));
		Peek.gs_water_central = Has.middle_water && (Has.longshot || (Has.can_use_farores && Has.hookshot) || Player.has_chus || Has.bow);
		Peek.gs_spirit_before_child_knuckle = Has.spirit_access && ((Has.bomb_bag || Player.has_chus) && (Has.boomerang || Has.slingshot || Player.has_chus) && Player.current_spirit_keys >= 1 && Has.child_spirit_access) || ((Has.hookshot || Has.bow || Player.has_chus || Has.can_use_dins) && Has.silver_gauntlets && Player.current_spirit_keys >= 1);
		Peek.gs_spirit_boulder_room = Has.adult_spirit_access && (Has.bow || Has.hookshot || Player.has_chus);
		Peek.gs_spirit_lobby = Has.spirit_access && (Has.adult_spirit_access && Player.current_spirit_keys >= 1 && (Has.hookshot || Has.hover_boots || Has.bow)) || ((Has.bomb_bag || Player.has_chus) && Has.slingshot && Player.current_spirit_keys >= 1 && Has.child_spirit_access);
		Peek.gs_shadow_crusher = Has.can_bomb_shadow_wall;
		Peek.gs_shadow_near_boat = Has.can_pass_shadow_hookshot_door && (Has.longshot || Has.bow || Player.has_chus) && Player.current_shadow_keys >=4;
		Peek.gs_well_west_inner = Has.well_access && Player.current_well_keys >=1;
		Peek.gs_well_east_inner = Has.well_access && Player.current_well_keys >=1;
		Peek.gs_ice_spinning_scythe = Has.ice_access && (Has.hookshot || Has.bow || Player.has_chus || Has.bomb_bag || Has.can_use_dins);
		Peek.gs_ice_hp_room = Has.ice_access && (Has.can_use_bottle || Has.can_shoot_blue_fire_arrows) && (Has.hookshot || Has.bow || Player.has_chus || Has.bomb_bag || Has.can_use_dins);
		Peek.gs_ice_block_room = Has.ice_access && (Has.can_use_bottle || Has.can_shoot_blue_fire_arrows) && (Has.hookshot || Has.bow || Player.has_chus || Has.bomb_bag || Has.can_use_dins);
		...
		if(document.getElementById("csmc").value == "ON") {
			...
			Peek.forest_blockRoom = Has.forest_temple_access && Player.current_forest_keys >= 1 && Has.bow;
			...
			Peek.water_block = Has.water_temple_access;
			Peek.water_river = Has.can_do_water_checks && Player.current_water_keys >= 2 && Has.time && Has.hookshot;
			Peek.spirit_adultLeft = Has.adult_spirit_access && Has.lullaby && (Has.bow || Has.hookshot || Has.bomb_bag || Player.has_chus);
			...
			Peek.gtg_freestanding = Has.gtg_access && (Player.current_gtg_keys >= 2 || (Has.bomb_bag || Player.has_chus) || (Has.can_climb_gtg_hole /*&& Has.can_see*/));
			Peek.gtg_right2 = Has.gtg_access;
			Peek.gtg_right3 = Has.gtg_access;
			Peek.gtg_left1 = Has.gtg_access && (Player.current_gtg_keys >= 1 || (Has.can_climb_gtg_hole && Has.bow));
			Peek.gtg_left2 = Has.gtg_access;
			Peek.gtg_left3 = Has.gtg_access;
			Peek.gtg_left4 = Has.gtg_access;
			Peek.gtg_final = Has.gtg_access;
			Peek.well_backBombable = Has.well_access;
			Peek.well_waterLeft = Has.well_access;
			Peek.well_frontBombable = Has.well_access;
			Peek.well_waterFront = Has.well_access;
			Peek.well_locked2 = Has.well_access;//&& Has.can_see ;
			Peek.well_basement = Has.well_access;
		}
