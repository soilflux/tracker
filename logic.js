...
function locationLogic(){
	...
	Location_Logic.jabu_map = Logic.can_hit_jabu_switch && Logic.boomerang && Logic.jabu_child_access;
	Location_Logic.jabu_compass = Logic.can_hit_jabu_switch && Logic.boomerang && Logic.jabu_child_access;
	Location_Logic.jabu_barinade = Logic.can_hit_jabu_switch && Logic.boomerang && Logic.jabu_child_access;
	Location_Logic.forest_first = Logic.forest_temple_access;
	Location_Logic.forest_stalfos = Logic.forest_temple_access;
	Location_Logic.forest_midCourtyard = Logic.forest_temple_adult_access && ((((Logic.time || Logic.bow || (Logic.hover_boots && Logic.forest_keys>=1)) && Logic.hookshot)) || (Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5));
	Location_Logic.forest_highCourtyard = Logic.forest_temple_access && (Logic.time || (Logic.forest_temple_adult_access && ((Logic.bow && Logic.hookshot) || (Logic.hover_boots && Logic.forest_keys>=1) || (Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5))));
	Location_Logic.forest_lowCourtyard = Logic.forest_temple_access && (Logic.time || (Logic.forest_temple_adult_access && ((Logic.bow && Logic.hookshot) || (Logic.hover_boots && Logic.forest_keys>=1) || (Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5))));
	Location_Logic.forest_blockRoom = Logic.forest_temple_adult_access && Logic.forest_keys >= 1 && (Logic.bow || (Logic.forest_temple_child_access && Logic.slingshot)) && Logic.goron_bracelet;
	Location_Logic.forest_bossKey = Logic.forest_temple_adult_access && Logic.forest_keys >= 2 && Logic.bow && Logic.goron_bracelet;
	Location_Logic.forest_floormaster = Logic.forest_temple_adult_access && ((Logic.bow && Logic.goron_bracelet && Logic.forest_keys >=2) || (Logic.hover_boots && Logic.forest_keys >=1));
	Location_Logic.forest_red = Logic.forest_temple_adult_access && Logic.goron_bracelet && Logic.bow && Logic.forest_keys>=3;
	Location_Logic.forest_bow = Logic.forest_temple_adult_access && Logic.goron_bracelet && Logic.forest_keys>=3;
	Location_Logic.forest_blue = Logic.forest_temple_adult_access && Logic.goron_bracelet && Logic.bow && Logic.forest_keys>=3;
	Location_Logic.forest_fallingCeiling = Logic.forest_temple_adult_access && Logic.goron_bracelet && (Logic.bow || Logic.can_use_dins) && Logic.forest_keys>=5;
	...
	Location_Logic.water_torches = Logic.can_do_water_checks && Logic.lullaby && (Logic.bow || Logic.can_use_dins || Logic.water_temple_child_access);
	...
	Location_Logic.spirit_childLeft = Logic.spirit_temple_child_access && (Logic.slingshot || Logic.boomerang);
	...
	Location_Logic.spirit_rotatingMirror2 = Logic.can_push_spirit_silver_block && Logic.spirit_keys >=3;
	Location_Logic.spirit_lullabyHand = Logic.can_push_spirit_silver_block && Logic.spirit_keys >=3 && Logic.lullaby;
	Location_Logic.spirit_lullabyHigh = Logic.can_push_spirit_silver_block && Logic.spirit_keys >=3 && Logic.lullaby && (Logic.hookshot || Logic.hover_boots);
	Location_Logic.spirit_nearFourArmos = Logic.can_push_spirit_silver_block && Logic.spirit_keys >=4 && Logic.bomb_bag && Logic.mirror_shield;
	...
	Location_Logic.gtg_lobbyLeft = Logic.gtg_access && ((Logic.bow && Logic.gtg_adult_access) || (Logic.slingshot && Logic.gtg_child_access));
	Location_Logic.gtg_lobbyRight = Logic.gtg_access && ((Logic.bow && Logic.gtg_adult_access) || (Logic.slingshot && Logic.gtg_child_access));
	Location_Logic.gtg_stalfos = Logic.gtg_access;
	Location_Logic.gtg_wolfos = Logic.gtg_adult_access && Logic.hookshot;
	Location_Logic.gtg_silvers1 = Logic.gtg_adult_access && Logic.hookshot && Logic.silver_gauntlets ;//&& Logic.can_see ;
	...
	Location_Logic.gtg_toilet = Logic.gtg_adult_access && Logic.iron_boots && Logic.time && Logic.hookshot /*&& Logic.can_see*/;
	Location_Logic.well_fakeRight = Logic.botw_child_access;//&& Logic.can_see;
	Location_Logic.well_centerSmall = Logic.botw_child_access;//&& Logic.can_see;
	Location_Logic.well_backBombable = Logic.botw_child_access && Logic.bomb_bag;
	Location_Logic.well_waterLeft = Logic.botw_child_access && Logic.lullaby;
	Location_Logic.well_coffin = Logic.botw_child_access;
	Location_Logic.well_centerBig = Logic.botw_child_access;//&& Logic.can_see;
	Location_Logic.well_fakeLeft = Logic.botw_child_access;//&& Logic.can_see;
	Location_Logic.well_frontBombable = Logic.botw_child_access && Logic.bomb_bag;
	...
	Location_Logic.shop_domain_TL = Logic[Shop_Logic.shop_domain_TL] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.jabu_entrance_access || (Logic.giants_wallet && Logic.bottle))));
	Location_Logic.shop_domain_TR = Logic[Shop_Logic.shop_domain_TR] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.jabu_entrance_access || (Logic.giants_wallet && Logic.bottle))));
	Location_Logic.shop_domain_BR = Logic[Shop_Logic.shop_domain_BR] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.jabu_entrance_access || (Logic.giants_wallet && Logic.bottle))));
	Location_Logic.shop_domain_BL = Logic[Shop_Logic.shop_domain_BL] && ((Logic.bomb_bag && Logic.lullaby) || Logic.silver_scale || (Logic.lullaby && ((Logic.can_enter_ganons && Logic.bottle) || Logic.jabu_entrance_access || (Logic.giants_wallet && Logic.bottle))));
	...
	Location_Logic.gs_fountain_above_log = Logic.jabu_entrance_access && Logic.boomerang;
	Location_Logic.gs_fountain_tree = Logic.jabu_entrance_access;
	...
	Location_Logic.gs_forest_first = Logic.forest_temple_adult_access && Logic.hookshot;
	...
	Location_Logic.gs_fire_basement = Logic.fire_temple_adult_access && Logic.hammer;
	...
	for(let i = 0, Access = Location_Access, Has = Player; i < 2; i++) {
		...
		Access.dodongos_end_of_bridge = Has.dodongos_climb && (Has.bomb_bag || Player.has_chus || (Has.dodongos_adult_access && Has.hammer));
		...
		Access.ice_map = Has.ice_access && (Has.can_use_bottle || Has.can_shoot_blue_fire_arrows) && (Has.ice_adult_access || Player.has_chus || Has.bomb_bag);
		Access.ice_hp = Has.ice_access && (Has.can_use_bottle || Has.can_shoot_blue_fire_arrows) && (Has.ice_adult_access || Player.has_chus || Has.bomb_bag || Has.giants_wallet);
		Access.ice_compass = Has.ice_access && (Has.can_use_bottle || Has.can_shoot_blue_fire_arrows) && (Has.ice_adult_access || Player.has_chus || Has.bomb_bag || Has.giants_wallet);
		Access.ice_irons = Has.ice_access && (Has.can_use_bottle || Has.can_shoot_blue_fire_arrows) && (Has.ice_adult_access || Player.has_chus || Has.bomb_bag || Has.giants_wallet);
		...
		Access.fire_lavaBomb = Has.fire_temple_access && Player.current_fire_keys >=1 && (Has.bomb_bag || (Player.has_chus && Has.fire_temple_adult_access)) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.fire_key_ring);
		...
		Access.water_morpha = Has.water_temple_adult_access && Has.water_boss_key /*&& Has.longshot*/ ;
		Access.spirit_childLeft = Has.spirit_temple_child_access && (Has.slingshot || Has.boomerang || Player.has_chus);
		Access.spirit_childRight = Has.spirit_temple_child_access && (Has.slingshot || Has.boomerang || Player.has_chus);
		...
		Access.spirit_map = Has.spirit_temple_access && Player.current_spirit_keys >= 1 && ((Has.spirit_temple_child_access && (Has.bomb_bag || Player.has_chus)) || (((Has.can_use_fire || Has.bow) && Has.silver_gauntlets))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_sunRoom = Has.spirit_temple_access && Player.current_spirit_keys >= 1 && ((Has.spirit_temple_child_access && (Has.bomb_bag || Player.has_chus)) || (((Has.can_use_fire || Has.bow) && Has.silver_gauntlets))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_rightHand = Has.spirit_temple_access && Player.current_spirit_keys >= 2 && ((Has.spirit_temple_child_access && (Has.bomb_bag || Player.has_chus)) || (Has.can_push_spirit_silver_block && (Has.bow || Has.hookshot || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_adultLeft = Has.can_push_spirit_silver_block && Has.hookshot && Has.lullaby;
		Access.spirit_adultRight = Has.can_push_spirit_silver_block && (Has.bow || Has.hookshot || Player.has_chus);
		...
		Access.spirit_nearFourArmos = Has.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && Has.mirror_shield && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		Access.spirit_invisible1 = Has.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring) ;//&& Has.can_see;
		Access.spirit_invisible2 = Has.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring) ;//&& Has.can_see;
		Access.spirit_leftHand = Has.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (Has.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		...
		Access.spirit_twinrova = Has.can_push_spirit_silver_block && Player.current_spirit_keys >=3 && Has.mirror_shield && (Has.bomb_bag || Player.has_chus) && Player.spirit_boss_key && Has.hookshot && (document.getElementById("keysanity").value != "KEY RINGS" || Has.spirit_key_ring);
		...
		Access.gtg_freestanding = Has.gtg_access && (Player.current_gtg_keys >= 2 || ((Has.bomb_bag || Player.has_chus) && (Has.time || Has.gtg_child_access)) || (Has.can_climb_gtg_hole /*&& Has.can_see*/ && Has.time));
		Access.gtg_right2 = Has.gtg_access && (Player.current_gtg_keys >= 2 || ((Has.bomb_bag || Player.has_chus) && (Has.time || Has.gtg_child_access)) || (Has.can_climb_gtg_hole /*&& Has.can_see*/ && Has.time));
		Access.gtg_right3 = Has.gtg_access && (Player.current_gtg_keys >= 2 || ((Has.bomb_bag || Player.has_chus) && (Has.time || Has.gtg_child_access)) || (Has.can_climb_gtg_hole /*&& Has.can_see*/ && Has.time));
		...
		Access.well_backBombable = Has.botw_child_access && (Has.bomb_bag || Player.has_chus);
		...
		Access.well_frontBombable = Has.botw_child_access && (Has.bomb_bag || Player.has_chus);
		...
		Access.well_locked1 = Has.botw_child_access && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);//&& Has.can_see ;
		Access.well_locked2 = Has.botw_child_access &&  (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);//&& Has.can_see ;
		Access.well_basement = Has.botw_child_access && ((Has.bomb_bag || Player.has_chus) || (((/*Has.can_see &&*/ Player.current_well_keys >= 1) || Has.can_use_dins) && Has.goron_bracelet));
		...
		Access.gs_spirit_metal_fence = Has.spirit_temple_child_access && (Has.boomerang || Has.slingshot || Player.has_chus);
		Access.gs_spirit_before_child_knuckle = Has.spirit_temple_access && ((Has.bomb_bag || Player.has_chus) && Has.boomerang && Player.current_spirit_keys >= 1 && Has.spirit_temple_child_access) || (Has.hookshot && Has.silver_gauntlets && Player.current_spirit_keys >= 1);
		Access.gs_spirit_boulder_room = Has.can_push_spirit_silver_block && Has.time && (Has.bow || Has.hookshot || Player.has_chus);
		...
		Access.gs_well_west_inner = Has.botw_child_access && Has.boomerang && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);
		Access.gs_well_east_inner = Has.botw_child_access && Has.boomerang && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);
		Access.gs_well_like_like = Has.botw_child_access && (Player.current_well_keys >= 1 || (Has.lullaby && (Has.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || Has.well_key_ring);
	...
	Location_Could_Access.fire_lavaBomb = CouldHave.fire_temple_access && (Player.current_fire_keys >=1 || (CouldHave.hammer && CouldHave.fire_temple_adult_access)) && (CouldHave.bomb_bag || (Player.has_chus && CouldHave.fire_temple_adult_access)) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	Location_Could_Access.fire_volvagia = CouldHave.fire_temple_adult_access && ((Player.fire_boss_key && CouldHave.hammer) || (CouldHave.hammer && CouldHave.bow && CouldHave.hookshot && (CouldHave.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.fire_key_ring);
	...
	Location_Could_Access.water_compass = CouldHave.water_temple_adult_access && ((CouldHave.lullaby && CouldHave.longshot) || CouldHave.iron_boots);
	...
	Location_Could_Access.spirit_childLeft = CouldHave.spirit_temple_child_access && (CouldHave.slingshot || CouldHave.boomerang || (CouldHave.bomb_bag || Player.has_chus));
	Location_Could_Access.spirit_childRight = CouldHave.spirit_temple_child_access && (CouldHave.slingshot || CouldHave.boomerang || (CouldHave.bomb_bag || Player.has_chus));
	...
	Location_Could_Access.spirit_rightHand = Player.current_spirit_keys >= 2 && ((CouldHave.can_push_spirit_silver_block && CouldHave.projectile_adult) || (CouldHave.spirit_temple_child_access && (CouldHave.bomb_bag || Player.has_chus))) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_adultLeft = CouldHave.can_push_spirit_silver_block && CouldHave.hookshot && CouldHave.lullaby;
	Location_Could_Access.spirit_adultRight = CouldHave.can_push_spirit_silver_block && (CouldHave.bow || CouldHave.hookshot || (CouldHave.bomb_bag || Player.has_chus));
	...
	Location_Could_Access.spirit_nearFourArmos = CouldHave.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && CouldHave.mirror_shield && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	Location_Could_Access.spirit_invisible1 = CouldHave.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring); //&& CouldHave.can_see;
	Location_Could_Access.spirit_invisible2 = CouldHave.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring); //&& CouldHave.can_see;
	Location_Could_Access.spirit_leftHand = CouldHave.can_push_spirit_silver_block && Player.current_spirit_keys >=2 && (CouldHave.bomb_bag || Player.has_chus) && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	...
	Location_Could_Access.spirit_twinrova = CouldHave.can_push_spirit_silver_block && Player.current_spirit_keys >=3 && CouldHave.mirror_shield && (CouldHave.bomb_bag || Player.has_chus) && Player.spirit_boss_key && CouldHave.hookshot && (document.getElementById("keysanity").value != "KEY RINGS" || CouldHave.spirit_key_ring);
	...
	for(let i = 0, Peek = Location_Peek, Has = Player; i < 2; i++) {
		...
		Peek.gs_fountain_above_log = Has.jabu_entrance_access && (Has.boomerang || Player.has_chus || Has.slingshot || Has.can_use_dins || Has.bomb_bag);
		...
		Peek.gs_forest_outdoor_west = Has.forest_temple_adult_access && (Has.hookshot || Has.bow || (Has.time && Player.has_chus));
		Peek.gs_water_central = Has.middle_water && (Has.longshot || (Has.can_use_farores && Has.hookshot) || Player.has_chus || Has.bow);
		Peek.gs_spirit_before_child_knuckle = Has.spirit_temple_access && ((Has.bomb_bag || Player.has_chus) && (Has.boomerang || Has.slingshot || Player.has_chus) && Player.current_spirit_keys >= 1 && Has.spirit_temple_child_access) || ((Has.hookshot || Has.bow || Player.has_chus || Has.can_use_dins) && Has.silver_gauntlets && Player.current_spirit_keys >= 1);
		Peek.gs_spirit_boulder_room = Has.can_push_spirit_silver_block && (Has.bow || Has.hookshot || Player.has_chus);
		Peek.gs_spirit_lobby = Has.spirit_temple_access && (Has.can_push_spirit_silver_block && Player.current_spirit_keys >= 1 && (Has.hookshot || Has.hover_boots || Has.bow)) || ((Has.bomb_bag || Player.has_chus) && Has.slingshot && Player.current_spirit_keys >= 1 && Has.spirit_temple_child_access);
		...
		if(document.getElementById("csmc").value == "ON") {
			...
			Peek.spirit_adultLeft = Has.can_push_spirit_silver_block && Has.lullaby && (Has.bow || Has.hookshot || Has.bomb_bag || Player.has_chus);
